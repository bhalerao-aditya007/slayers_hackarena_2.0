...
class InvestmentGoal(BaseModel):
    return_target: float = Field(..., ge=0.0, le=5.0, description="Annual return target as fraction (e.g. 0.15 for 15%)")
    max_drawdown: float = Field(..., ge=0.0, le=1.0, description="Max acceptable drawdown (e.g. 0.10 for 10%)")
    sectors_excluded: list[str] = Field(default_factory=list)
    capital_inr: float = Field(..., gt=0, description="Investment capital in INR")
    ...
    def clamp_return_target(cls, v: float) -> float:
        if v > 2.0:
            import logging
            logging.warning("return_target=%s exceeds 200%%; clamping to 2.0", v)
        return min(v, 2.0)
...
class AnalyzeRequest(BaseModel):
    nl_goal: str = Field(..., min_length=10, max_length=2000)
    portfolio: list[TickerWeight] = Field(default_factory=list)
...
class ScenarioRequest(BaseModel):
    ...
    custom_shock: dict[str, float] | None = None   # ticker → shock magnitude
...
class ScenarioResult(BaseModel):
    ...
    contagion_path: dict[str, float]   # ticker → shocked return
